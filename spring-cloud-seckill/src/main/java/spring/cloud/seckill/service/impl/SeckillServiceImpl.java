package spring.cloud.seckill.service.impl;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    @Override
    public List<SeckillModel> findAll() {
        List<SeckillModel> seckillList = redisTemplate.boundHashOps("seckill").values();
        if (seckillList == null || seckillList.size() == 0){
            //说明缓存中没有秒杀列表数据
            //查询数据库中秒杀列表数据，并将列表数据循环放入redis缓存中
            seckillList = seckillMapper.findAll();
            for (SeckillModel seckill : seckillList){
    ...
    @Override
    public SeckillModel findById(Long seckillId) {
        return seckillMapper.findById(seckillId);
    }

    @Override
    public Exposer exportSeckillUrl(Long seckillId) {
        SeckillModel seckill = (SeckillModel) redisTemplate.boundHashOps(key).get(seckillId);
        if (seckill == null) {
        ...
        try {
            ...
            if (insertCount <= 0) {
                ...
                if (updateCount <= 0) {
                    ...
                    //秒杀成功
                    SeckillOrderModel seckillOrder = seckillOrderMapper.findById(seckillId, userPhone);
