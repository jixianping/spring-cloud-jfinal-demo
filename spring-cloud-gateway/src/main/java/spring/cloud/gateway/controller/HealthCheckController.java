...
import spring.cloud.gateway.feignService.GoodsHealthCheckService;
...
public class HealthCheckController {
    ...
    @Autowired private GoodsHealthCheckService goodsHealthCheckService;
    ...
    public String traceHealth(HttpServletResponse response) {
        ...
        String secHealth = this.goodsHealthCheckService.health();
        if ( FAIL.equalsIgnoreCase(  secHealth ) ) {
            LOGGER.error("goodsHealthCheckService.health fail");
            response.setStatus(500);
            return FAIL;
        }
